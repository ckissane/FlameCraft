...
public class FlameGenerator extends GraphicsProgram {
	...
	public void init() {
		coeffs = new ArrayList<ArrayList<Double>>(coeffCount);
		colorOff = 0.1F;
	...
	public void run() {
		...
		while (genThread.isAlive()) {
			/*
			 * for(int i=0;i<1000;i++){ int choice = new
			 * Random().nextInt(coeffCount-1); transform(choice,1);
			 * if(steps==0){ c = 0F; } if(c==0F){ c=choice/coeffCount; }
			 * c=(c+choice/coeffCount)/2; Color hsb=Color.getHSBColor(c, 1F,
			 * 50); plotPoint(px.doubleValue(),py.doubleValue(),new
			 * Color(hsb.getRed(),hsb.getBlue(),hsb.getGreen(),50)); }
			 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("reRun".equals(e.getActionCommand())) {
			this.init();
	...
	public void plotPoint(double x, double y, Color color) {
		...
		imgGraphics.drawLine((int) (x * halfway + halfway),
				(int) (y * halfway + halfway), (int) (x * halfway + halfway),
	...
	private class GeneratingThread extends Thread {
				...
				for (int i = 0; i < 100000; i++) {
					...
					if (steps > 1) {
						boss.plotPoint(
								px,
								py,
								new Color(hsb.getRed(), hsb.getGreen(), hsb
										.getBlue(), 50));
					}
					/*
					 * transform(choice,2); if(steps>0){
					 * boss.plotPoint(px,py,new
					 * Color(hsb.getRed(),hsb.getGreen(),hsb.getBlue(),50)); }
					 */
				}
			}
			this.stop();
		...
		public void transform(int coeffID, int method) {
			try {
				...
				double x = px * coeffShort.get(0) + py * coeffShort.get(1)
						+ coeffShort.get(2);
				double y = px * coeffShort.get(3) + py * coeffShort.get(4)
						+ coeffShort.get(5);
				variate(x, y, method);
			} catch (Exception e) {
		...
		public void variate(double x, double y, int method) {
			...
			if (method == 1) {
				px = x * Math.sin(r);// .max(new BigDecimal(-1)).min(new
										// BigDecimal(1));
				py = y * Math.cos(r);// .max(new BigDecimal(-1)).min(new
										// BigDecimal(1));
			}
			if (method == 2) {
				px = x * (1 / r);// .max(new BigDecimal(-1)).min(new
									// BigDecimal(1));
				py = y * (1 / r);// .max(new BigDecimal(-1)).min(new
									// BigDecimal(1));
			}
			if (method == 3) {
